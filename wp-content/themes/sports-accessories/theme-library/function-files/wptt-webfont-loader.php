...
if ( ! class_exists( 'Sports_Accessories_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Sports_Accessories_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $sports_accessories_url The remote URL.
		 */
		public function __construct( $sports_accessories_url = '' ) {
			$this->remote_url = $sports_accessories_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $sports_accessories_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $sports_accessories_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $sports_accessories_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $sports_accessories_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $sports_accessories_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $sports_accessories_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $sports_accessories_format = 'woff2' ) {
			$this->font_format = $sports_accessories_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $sports_accessories_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $sports_accessories_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $sports_accessories_url, '/' ) ) {
				$sports_accessories_parsed_url = parse_url( $this->remote_url );
				return $sports_accessories_parsed_url['scheme'] . '://' . $sports_accessories_parsed_url['hostname'] . $sports_accessories_url;
			}

			return $sports_accessories_url;
		}
	}
}

if ( ! function_exists( 'sports_accessories_wptt_get_webfont_styles' ) ) {
	/**
	 ...
	 * @since 1.0.0
	 *
	 * @param string $sports_accessories_url    The URL of the remote webfont.
	 * @param string $sports_accessories_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function sports_accessories_wptt_get_webfont_styles( $sports_accessories_url, $sports_accessories_format = 'woff2' ) {
		$sports_accessories_font = new Sports_Accessories_WPTT_WebFont_Loader( $sports_accessories_url );
		$sports_accessories_font->set_font_format( $sports_accessories_format );
		return $sports_accessories_font->get_styles();
	}
}

if ( ! function_exists( 'sports_accessories_wptt_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function sports_accessories_wptt_get_webfont_url( $sports_accessories_url, $sports_accessories_format = 'woff2' ) {
		$sports_accessories_font = new Sports_Accessories_WPTT_WebFont_Loader( $sports_accessories_url );
		$sports_accessories_font->set_font_format( $sports_accessories_format );
		return $sports_accessories_font->get_url();
	}
}